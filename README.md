...
- 自动价格：默认读取中国银行“印尼卢比”现汇卖出价，并按 `IDR/CNY = 100 ÷ 现汇卖出价` 换算；`FX_PROVIDER=auto` 也会优先使用中国银行。其他行情源仍可显式选择。自动售价可在系统状态中选择“买入价 + 固定 IDR”或“买入价 + 百分比”，同时支持单 SKU 手动售价。后台手动汇率会在下一次在线刷新时被覆盖。
...
`SUPPLIER_PRODUCT_TYPES=topup` 使用 ReloadN 文档明确提供的完整充值目录。若 ReloadN 后续为商户开通其他商品类型，可用英文逗号追加类型；只有供应商明确支持无类型查询时才使用 `all`。商品同步会继续跟随游标、页码、offset，供应商只返回 `total + limit` 时也会按累计 offset 拉完。后台每页最多渲染 200 个 SKU，供应商原始响应不会发送到浏览器。
...
生产环境默认使用中国银行外汇牌价中的“印尼卢比现汇卖出价”。中国银行页面以“100 印尼卢比兑换多少人民币”报价，因此系统使用以下口径换算：

```text
IDR/CNY = 100 ÷ 印尼卢比现汇卖出价
```

例如现汇卖出价为 `0.0385`，则 `IDR/CNY = 100 ÷ 0.0385 = 2597.4026`。现汇卖出价代表购入印尼卢比时使用的人民币卖出侧价格，适合本项目的保守定价口径。

在服务器 `.env` 中配置：

```dotenv
FX_PROVIDER=bank-of-china
BOC_FX_URL=https://www.boc.cn/sourcedb/whpj/
BOC_REFRESH_MINUTES=60
EXCHANGE_RATE_API_KEY=你的服务端密钥
EXCHANGE_RATE_API_REFRESH_MINUTES=60
COINBASE_REFRESH_MINUTES=60
OPEN_ER_API_URL=https://open.er-api.com/v6/latest/CNY
OPEN_ER_API_REFRESH_MINUTES=480
```

`FX_PROVIDER=bank-of-china` 显式使用中国银行；`FX_PROVIDER=auto` 同样默认优先使用中国银行。`BOC_FX_URL` 是中国银行外汇牌价页面，`BOC_REFRESH_MINUTES` 控制建议刷新间隔。若需切换其他数据源，可显式设置 `exchange-rate-api`、`coinbase` 或 `open-er-api`，并配置对应地址、密钥与刷新间隔。管理后台只返回数据源名称、报价时间、抓取时间和建议刷新间隔，不会返回密钥或包含密钥的请求地址。定时任务每 30 分钟检查一次是否到达数据源建议的刷新时间；刷新失败时保留上一笔有效报价，并继续按恢复间隔重试。
