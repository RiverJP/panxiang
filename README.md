...
- 自动价格：配置 `EXCHANGE_RATE_API_KEY` 后优先使用 ExchangeRate-API 的 CNY/IDR 行情；未配置密钥时使用 Coinbase 无密钥当前汇率快照，两者默认每 60 分钟刷新。open.er-api 免费日更行情仍可显式选作降级源。自动售价可在系统状态中选择“买入价 + 固定 IDR”或“买入价 + 百分比”，同时支持单 SKU 手动售价。后台手动汇率会在下一次在线刷新时被覆盖。
...
`FX_PROVIDER=auto` 会在存在密钥时使用 ExchangeRate-API；没有密钥时使用 Coinbase 官方 `/v2/exchange-rates?currency=CNY` 当前快照，不需要把任何行情密钥放进前端。Coinbase 响应不提供上游报价时间，因此系统会将安全收到快照的时间同时记录为行情快照时间和抓取时间。若要显式使用免费日更降级源，可设置 `FX_PROVIDER=open-er-api`。管理后台只返回数据源名称、报价时间、抓取时间和建议刷新间隔，不会返回密钥或包含密钥的请求地址。定时任务每 30 分钟检查一次是否到达数据源建议的刷新时间；刷新失败时保留上一笔有效报价，并继续按恢复间隔重试。
